...
{
    /// <summary>
    /// Парсер регистров для контроллера CREVIS.
    /// </summary>
    public class CrevisParser : IRegisterParser<Variable>
    {
        private readonly Action<string> _errorLog;
        private readonly Action<string> _infoLog;

        public CrevisParser(Action<string> errorLog = null, Action<string> infoLog = null)
        {
            _errorLog = errorLog;
            _infoLog = infoLog;
        }
        ...
        public IParserResult<Variable> GetFromCsvText(string text, ushort start)
        {
                        ...
                        if (!line.Contains(";;"))
                        {
                            ...
                            switch (type)
                            {
                                ...
                                case VariableType.Int:
                                    CreateVariable(
                                           var_name,
                                           (name) => registers.Add(name, new BoolVariable(name, adr, mask, accessLevel, description)),
        ...
        private void CreateVariable(string var_name, Action<string> boolAddAction, Action otherAddAction)
        {
            if (var_name.Contains("_BOOL"))
            {
                string newName = var_name.Replace("_BOOL", "");
                boolAddAction?.Invoke(newName);
                return;
            }
            otherAddAction?.Invoke();
        }
    }
}
